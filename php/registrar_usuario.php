...
try {
    ...
    if (isset($_FILES['avatar2D']) && $_FILES['avatar2D']['error'] === 0) {
        ...
        // Si no se sube un avatar, lanzamos un error.
        // Esto es para forzar la subida de un avatar durante el registro.
        // Si se quisiera permitir el registro sin avatar, aquí se asignaría uno por defecto.
        throw new Exception('Es obligatorio subir un avatar.');
    }
    ...
    $stmt = $conn->prepare("
        INSERT INTO users 
        (nombre, sexo, email, password, enlaces, descripcion, ciudad, provincia, modalidad, avatar2D, avatar3D)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ");
